...
interface DocumentActions {
  ...
  createTemplate: (
    data: Partial<DocumentTemplateInsert> & { pages?: TemplatePage[] }
    ...
    subscribeWithSelector(
      immer((set, get) => ({
                    ...
                    const refreshedPages = await Promise.all(
                      template.pages.map(async (page: TemplatePage) => {
                        if (page.image_path) {
                          ...
                          if (urlError) {
                            console.error(
                              'Error creating signed URL for page:',
                              page.image_path,
                              urlError
                            )
                          }
        ...
        uploadTemplateImages: async (files) => {
          const results = await Promise.all(files.map((file) => get().uploadTemplateImage(file)))
          return results
        },
        ...
        updatePageFields: async (templateId, pageId, fields) => {
          ...
          const updatedPages = pages.map((p, i) => (i === pageIndex ? { ...p, fields } : p))
