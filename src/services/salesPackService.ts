...
async function pdfPageToImage(
  ...
  console.log(`[SalesPack] Rendering page ${pageNum}: ${canvas.width}x${canvas.height} at scale ${scale}`);
...
async function classifyPagesWithClaude(pageTexts: string[]): Promise<BatchClassifyResult> {
  ...
  pageTexts.forEach((text, i) => {
    console.log(`[SalesPack] Page ${i + 1}: ${text.length} chars - "${text.substring(0, 100).replace(/\n/g, ' ')}..."`);
  });
  ...
  console.log(`[SalesPack] Batch classification result:`, {
    ...
    groups: data.documentGroups?.map((g: { documentType: string; pages: number[] }) => `${g.documentType}(pages:${g.pages.join(',')})`),
  });
...
async function analyzeWithClaudeVision(
...
): Promise<{ pageTexts: string[]; totalPages: number; customerName: string; documentGroups: BatchClassifyResult['documentGroups'] }> {
  const supabase = getSupabase();
  ...
  if (pdfSizeMB > 20) {
    throw new Error(`PDF is too large (${pdfSizeMB.toFixed(1)}MB). Please use a smaller PDF or split it first.`);
  }
...
export async function analyzeSalesPack(
  ...
  for (let i = 0; i < pageClassifications.length; i++) {
    ...
    if (claudeResult) {
      ...
      pageClassifications[i].documentTypeName = claudeResult.documentTypeName ||
        getDocumentTypeInfo(claudeResult.documentType).name;
      pageClassifications[i].confidence = claudeResult.confidence;
    }
  }

  // Step 3: Generate suggested splits based on Claude's grouping
  const suggestedSplits: SplitDocument[] = classificationResult.documentGroups.map((group, idx) => ({
    id: `doc-${idx}-${Date.now()}`,
    documentType: group.documentType,
    documentTypeName: group.documentTypeName || getDocumentTypeInfo(group.documentType).name,
    pages: group.pages,
