...
export async function uploadCustomerDocument(
  ...
  // Upload file
  console.log(
    `[Upload] Starting upload: ${file.name} (${(file.size / 1024).toFixed(1)}KB) -> ${filePath}`
  )
...
export async function getCustomerDocuments(
  ...
  const basePath = documentType ? `${userId}/${safeName}/${documentType}` : `${userId}/${safeName}`
...
export async function deleteAllCustomerDocuments(
  ...
  const supabase = getSupabase()
  const { data: files, error: listError } = await supabase.storage.from(BUCKET_NAME).list(basePath)

  if (listError) throw listError

  if (files && files.length > 0) {
    const paths = files.filter((f) => f.id !== null).map((f) => `${basePath}/${f.name}`)
 ...
 */
export async function getCustomerDocumentFolders(customerName: string): Promise<string[]> {
  const userId = await getAuthenticatedUserId()
  const safeName = sanitizeCustomerName(customerName)
  ...
  if (result.data) {
    ...
    return result.data.filter((f) => f.id === null).map((f) => f.name)
  }
...
export async function deleteEntireCustomerFolder(customerName: string): Promise<void> {
  ...
  if (allFilePaths.length > 0) {
    ...
    const { error: deleteError } = await supabase.storage.from(BUCKET_NAME).remove(allFilePaths)
