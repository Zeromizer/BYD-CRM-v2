...
function convertCustomer(old: OldCustomer): Omit<CustomerInsert, 'user_id'> {
  ...
  if (checklist && 'testDrive' in checklist) {
    ...
    checklist = {
      ...
      test_drive:
        ((checklist as Record<string, unknown>).testDrive as Record<string, boolean>) ?? {},
      close_deal:
        ((checklist as Record<string, unknown>).closeDeal as Record<string, boolean>) ?? {},
      registration:
        ((checklist as Record<string, unknown>).registration as Record<string, boolean>) ?? {},
 ...
 */
function convertGuarantor(
  old: OldGuarantor,
  position: 1 | 2 | 3 | 4 | 5
): Omit<GuarantorInsert, 'customer_id'> {
  return {
    position,
    name: old.name ?? '',
    phone: old.phone ?? null,
...
export function convertCustomers(data: OldExportData): CustomerImportResult {
  ...
  for (const oldCustomer of data.customers) {
    try {
      ...
      result.errors.push(
        `Failed to convert customer "${oldCustomer.name || 'Unknown'}": ${(err as Error).message}`
      )
    }
  }
...
export function exportCustomersToJSON(
  customers: Customer[],
  guarantorsMap: Record<number, Guarantor[]>
): string {
  const exportData = {
    version: '2.0',
