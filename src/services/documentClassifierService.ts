...
export async function classifyDocuments(
  ...
  debug.log(
    `[Queue] Starting classification for ${files.length} files (${totalBatches} batches, size ${BATCH_SIZE})`
  )
  ...
  for (let i = 0; i < files.length; i += BATCH_SIZE) {
    ...
    debug.log(
      `[Queue] Processing batch ${batchNum}/${totalBatches}: ${batch.map((f) => f.name).join(', ')}`
    )
