...
const DOCUMENT_TYPE_INFO: Record<string, { name: string; folder: string; milestone: string }> = {
  ...
  insurance_acceptance: { name: 'Insurance Acceptance', folder: 'Insurance', milestone: 'registration' },
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const headerStr = headers.filter(h => h != null).map(h => String(h)).join(', ');
...
async function classifyPdfWithVisionClaude(file: File): Promise<VisionClaudeResult> {
  ...
  console.log(`Multi-page PDF (${numPages} pages), extracting text from each page with Vision API...`);
...
export async function extractVsaFormData(file: File): Promise<OcrResult<VsaFormData>> {
  ...
  const structuredData: VsaFormData = {
    ...
    validation: {
      ...
      allFieldsPresent: !!(result.extractedData.name && result.extractedData.vehicleModel && sellingPrice > 0),
      calculatedTotal,
      issues: [],
    },
  };
...
function classifyExcelContent(
  ...
  const hasCustomerFields =
    ...
    lowerHeaders.some((h) => safeIncludes(h, 'nric') || safeIncludes(h, 'ic') || safeIncludes(h, 'phone') || safeIncludes(h, 'contact'));

  // Check for vehicle/sales indicators
  const hasVehicleFields =
    lowerHeaders.some((h) => safeIncludes(h, 'vehicle') || safeIncludes(h, 'model') || safeIncludes(h, 'car')) ||
  ...
  const hasFinancialFields =
    lowerHeaders.some((h) => safeIncludes(h, 'price') || safeIncludes(h, 'amount') || safeIncludes(h, 'deposit') || safeIncludes(h, 'loan'));

  // Check for insurance indicators
  const hasInsuranceFields =
    lowerHeaders.some((h) => safeIncludes(h, 'insurance') || safeIncludes(h, 'premium') || safeIncludes(h, 'policy'));
