    ...
    img.onload = () => {
      ...
      if (selectedCustomer) {
        ...
        Object.entries(currentPage.fields ?? {}).forEach(([, field]) => {
          const value =
            field.type === 'custom' ? field.customValue : (customerData[field.type] ?? '')
  ...
  const getCustomerDataMapping = (customer: Customer): Record<string, string> => {
    ...
    return {
      ...
      invoiceInstallmentConditional:
        (customer.vsa_interest ?? 0) > 2.5 ? formatCurrency(customer.vsa_monthly_repayment) : '',
                ...
                {backPagePhotos.map((photo, index) => (
                  <div key={photo.id} className="pm-selected-photo">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setBackPagePhotos((prev) => prev.filter((_, i) => i !== index))
                      }
                    >
                      <Trash size={14} />
                    </button>
