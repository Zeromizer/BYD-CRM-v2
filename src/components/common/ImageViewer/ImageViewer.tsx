...
export function ImageViewer({ url, filename, onDownload }: ImageViewerProps) {
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing -- Fallback on empty string in template
      const title = filename || 'Print Image'
    ...
    <div className="image-viewer">
      ...
      <div className="image-toolbar">
        <div className="image-toolbar-left">
          <button className="image-toolbar-btn" onClick={handleRotate} title="Rotate 90°">
            <ArrowCounterClockwise
              size={18}
              weight="bold"
          ...
          <button className="image-toolbar-btn" onClick={handleResetZoom} title="Reset">
            <span className="toolbar-text">Reset</span>
          </button>
        </div>

        <div className="image-toolbar-right">
          <button className="image-toolbar-btn" onClick={handleZoomOut} title="Zoom out">
            <MagnifyingGlassMinus
              size={18}
              weight="bold"
              style={{ color: 'var(--text-secondary)' }}
          ...
          <button className="image-toolbar-btn" onClick={handleZoomIn} title="Zoom in">
            <MagnifyingGlassPlus
              size={18}
              weight="bold"
              style={{ color: 'var(--text-secondary)' }}
          ...
          <button className="image-toolbar-btn" onClick={handlePrint} title="Print">
            <Printer size={18} weight="bold" style={{ color: 'var(--text-secondary)' }} />
          </button>
          {onDownload && (
            <button className="image-toolbar-btn" onClick={onDownload} title="Download">
      ...
      {/* Filename footer */}
      {filename && <div className="image-filename-footer">{filename}</div>}
    </div>
  )
}
