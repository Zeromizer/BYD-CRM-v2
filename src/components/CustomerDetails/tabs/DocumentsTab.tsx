...
import { classifyDocuments, type ClassificationResult } from '@/services/documentClassifierService'
import {
...
import { getTemplatePages, getTotalFieldCount } from '@/types'
...
const DOCUMENT_CATEGORIES = [
  {
    ...
    documents: [
      ...
      { id: 'nric_back', label: 'NRIC Back', alternateIds: ['nric', 'id_documents'] },
      {
        ...
        alternateIds: ['driving_license_front', 'driving_license'],
      },
      {
        id: 'license_back',
        label: 'Driving License Back',
        alternateIds: ['driving_license_back', 'driving_license'],
      },
    ],
  },
  {
...
export function DocumentsTab({ customer }: DocumentsTabProps) {
  ...
  const [docToDelete, setDocToDelete] = useState<{ docId: string; doc: CustomerDocument } | null>(
    null
  )
  ...
  const [classifiedFiles, setClassifiedFiles] = useState<
    { file: File; name: string; classification: ClassificationResult | VisionClaudeResult }[]
  >([])
  ...
  const [allUploads, setAllUploads] = useState<
    { documentType: string; documents: CustomerDocument[] }[]
  >([])
  ...
  const handleStartMigration = async () => {
    ...
    try {
      const result = (await migrateCustomerDocuments(
        ...
        failed: number
        errors: string[]
        updatedChecklist?: DocumentChecklistState
      }
  ...
  const handleAIScan = async () => {
    ...
    try {
      ...
      let results: {
        file: File
        name: string
        classification: ClassificationResult | VisionClaudeResult
      }[]
      ...
      if (scanMethod === 'vision-claude') {
        ...
        const visionResults = await classifyDocumentsWithVisionClaudeParallel(
          ...
          (current, total, filename, result) => {
            setScanProgress(
              `[Vision+Claude] ${current}/${total}: ${filename}${result ? ` → ${result.documentTypeName}` : ''}`
            )
          },
        ...
        const claudeResults = await classifyDocuments(
          ...
          (current, total, filename, result) => {
            setScanProgress(
              `[Claude] ${current}/${total}: ${filename}${result ? ` → ${result.documentTypeName}` : ''}`
            )
          }
        )
  ...
  return (
    <div className="documents-tab">
      ...
      {isMobile ? (
        <>
                  ...
                  if (docs.length === 0 && doc.alternateIds) {
                    for (const altId of doc.alternateIds) {
                      if (uploadedDocs[altId] && uploadedDocs[altId].length > 0) {
                        ...
                        if (
                          altDocs.length > 1 &&
                          (doc.id.includes('front') || doc.id.includes('back'))
                        ) {
                          const keyword = doc.id.includes('front') ? 'front' : 'back'
                          const matched = altDocs.filter((d) =>
          ...
          {showAddDocSheet &&
            createPortal(
              <>
                <div className="mobile-action-overlay" onClick={() => setShowAddDocSheet(false)} />
              ...
              <>
                ...
                <div className="mobile-action-sheet doc-action-sheet">
                  <div className="doc-action-sheet-header">
                    <span className="doc-action-sheet-title">{selectedDocForAction.label}</span>
            ...
            {DOCUMENT_CATEGORIES.map((category) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`category-progress ${isComplete ? 'complete' : ''}`}>
                    ...
                    <div className="selection-toolbar">
                      <div className="selection-info">
                        <input
                          ...
                          checked={
                            selectedDocs.size > 0 &&
                            selectedDocs.size === allUploads.flatMap((f) => f.documents).length
                          }
                          ...
                          {folder.documents.map((doc) => (
                            ...
                            >
                              ...
                              <div className="document-card-info">
                                <div className="document-card-name" title={doc.name}>
                                  {doc.name}
                                </div>
                ...
                {activeDocuments.map((doc) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="document-card-info">
                        <div className="document-card-name" title={doc.label}>
                          {doc.label}
                        </div>
                        {docs.length > 0 && (
      ...
      <Modal
        ...
        title={
          showClassificationResults ? 'AI Classification Results' : 'Import Documents from Folder'
        }
        size="lg"
      >
          ...
          {!showClassificationResults ? (
            ...
            <>
              <p className="migration-description">
                Found <strong>{migrationFiles.length}</strong> documents to import for{' '}
              ...
              {migrationFiles.length > 0 && !isScanning && !isMigrating && (
                <div className="scan-method-selector">
                  ...
                  <div className="scan-method-options">
                    <label
                      className={`scan-method-option ${scanMethod === 'claude' ? 'selected' : ''}`}
                    >
                      <input
                        type="radio"
                        name="scanMethod"
                      ...
                      <div className="scan-method-content">
                        ...
                        <span className="scan-method-desc">
                          Current method - direct image analysis
                        </span>
                      </div>
                    </label>
                    <label
                      className={`scan-method-option ${scanMethod === 'vision-claude' ? 'selected' : ''}`}
                    >
                      <input
                        type="radio"
                        name="scanMethod"
                      ...
                      />
                      <div className="scan-method-content">
                        <span className="scan-method-name">
                          Vision + Claude <span className="testing-badge">Testing</span>
                        </span>
                        <span className="scan-method-desc">
                          Google Vision OCR + Claude - higher accuracy text extraction
                        </span>
                      </div>
                    </label>
                  </div>
              ...
              <div className="confirmation-actions">
                <Button variant="outline" onClick={handleCancelMigration} disabled={isMigrating}>
                  Cancel
                </Button>
                <Button
                  variant="outline"
            ...
            <>
              <p className="migration-description">
                ...
                classified <strong>{classifiedFiles.length}</strong> documents. Review and adjust if
                needed:
              </p>
              ...
              <div className="classification-results">
                <div className="file-list-scroll">
                  {classifiedFiles.map((item, index) => {
                    ...
                    const visionResult = hasOcrMethod
                      ? (item.classification as VisionClaudeResult)
                      : null
                    ...
                    return (
                      <div key={index} className="classified-file-item">
                        <div className="classified-file-info">
                          ...
                          <span
                            className={`confidence ${item.classification.confidence >= 75 ? 'high' : item.classification.confidence >= 50 ? 'medium' : 'low'}`}
                          >
                        ...
                        {visionResult?.extractedData && (
                          <div className="extracted-data">
                            {visionResult.extractedData.nric && (
                              <span className="extracted-field">
                                NRIC: {visionResult.extractedData.nric}
                              </span>
                            )}
                            {visionResult.extractedData.vehicleModel && (
                              <span className="extracted-field">
                                Vehicle: {visionResult.extractedData.vehicleModel}
                              </span>
                            )}
                            {visionResult.extractedData.phone && (
                              <span className="extracted-field">
                                Phone: {visionResult.extractedData.phone}
                              </span>
                            )}
                          </div>
                        )}
                        ...
                        {isExcelParse && visionResult?.excelData && (
                          <div className="excel-info">
                            <span className="excel-stats">
                              {visionResult.excelData.rowCount} rows,{' '}
                              {visionResult.excelData.columnCount} cols
                              {visionResult.excelData.sheetNames.length > 1 &&
                                ` (${visionResult.excelData.sheetNames.length} sheets)`}
                            </span>
                            {visionResult.excelData.headers &&
                              visionResult.excelData.headers.length > 0 && (
                                <details className="excel-headers">
                                  <summary>
                                    Headers: {visionResult.excelData.headers.slice(0, 3).join(', ')}
                                    {visionResult.excelData.headers.length > 3 ? '...' : ''}
                                  </summary>
                                  <div className="headers-list">
                                    {visionResult.excelData.headers.join(', ')}
                                  </div>
                                </details>
                              )}
                          </div>
                        ...
                        {visionResult?.rawText && !isExcelParse && (
                          <details className="raw-text-preview">
                            ...
                            <pre>
                              {visionResult.rawText.substring(0, 500)}
                              {visionResult.rawText.length > 500 ? '...' : ''}
                            </pre>
                          </details>
                        )}
                        ...
                        {isExcelParse && visionResult?.rawText && (
                          <details className="raw-text-preview">
                            ...
                            <pre>
                              {visionResult.rawText.substring(0, 800)}
                              {visionResult.rawText.length > 800 ? '...' : ''}
                            </pre>
                          </details>
                        )}
                      </div>
